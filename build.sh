...
if [ "$1" = "dev" ]; then
  ...
  webVersion=$(wget -qO- -t1 -T2 "https://api.github.com/repos/popo1221/alist-web/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
fi
...
FetchWebDev() {
  curl -L https://codeload.github.com/popo1221/web-dist/tar.gz/refs/heads/dev -o web-dist-dev.tar.gz
...
FetchWebRelease() {
  curl -L https://github.com/popo1221/alist-web/releases/latest/download/dist.tar.gz -o dist.tar.gz
